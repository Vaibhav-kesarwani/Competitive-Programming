 ...
 * Created       :         December 02, 2024 20:35:03
 * Workspace     :         ~/Desktop/Competitive-Programming
 * Problem name  :         B_Game_with_Colored_Marbles
*/
...
#define fora(a) for (auto i : a) // for the linear traversal
...
void Solve_Karo_Jaldi_Sa_Dusra_Bhi_Karna_Hai() {
    ...
    fora(m1) {
        vi &temp = i.se;
        ((i.fi == 1 && 1) ? ans += (((sz(temp) / 2) + (sz(temp) % 2)) * 2) : ans += sz(temp));
    }
    cout << ans nt
}
...
signed main() {
    Code By Vaibhav
    int Bhai_Test_Case_Hai_Ya; cin >> Bhai_Test_Case_Hai_Ya; while (Bhai_Test_Case_Hai_Ya--) 
