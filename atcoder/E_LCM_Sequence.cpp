...
#define forc(i, l, r) for (ll i = (int)(l); i <= (int)(r); ++i)
...
void help() {
    ...
    forc(i, 2, MAXIMP) {
        if (isPrime [i]) {
            primes.pb(i);
            for (ll j = i * i; j <= MAXIMP; j += i) isPrime [j] = false;
        }
    }
}
...
void Solve_Karo_Jaldi_Sa_Dusra_Bhi_Karna_Hai() {
    ...
    fora(primes) {
        for (ll x = i * i; x <= r; x *= i) {
            if (x > l) st.insert(x);
            if (x > r / i) break;
        }
    }

    vb sePrime(r - l + 1, true);
    fora(primes) {
        for (ll j = max(i * i, ((l + i - 1) / i) * i); j <= r; j += i) sePrime[j - l] = false;
    }

    for0(i, r - l + 1) {
        if (sePrime[i] && l + i > l) st.insert(l + i);
    }
