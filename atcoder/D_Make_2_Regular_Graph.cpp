...
int cntOp(const set<ii>& target) {
    int add = 0, rem = 0;

    fora(target) if (!ie.count(i)) add++;
    fora(ie) if (!target.count(i)) rem++;

    return add + rem;
}

void generate(int n, int start, vi& cur, vvi& ans) {
    if (n == 0) {
        ans.pb(cur);
        return;
    }
    ...
    for (const auto& it : part) {
        ...
        do {
            ...
            for (int len : it) {
                ...
                vi cycle;
                for0(i, len) cycle.pb(nodes[idx++]);

                for0(i, len) {
                    int u = cycle[i], v = cycle[(i + 1) % len];
                    if (u > v) swap(u, v);
                    edges.insert({u, v});
                }
            }
