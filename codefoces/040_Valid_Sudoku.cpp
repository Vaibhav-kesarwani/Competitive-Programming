...
bool isValid(vector<vector<int>>& a) {
    for0(i, 9) {
        ...
        for0(j, 9) {
            ...
            if (row < 1 or row > 9 or col < 1 or col > 9) return 0;
            if (rs.count(row) or cs.count(col)) return 0;
    ...
    for (int row = 0; row < 9; row += 3) {
        for (int col = 0; col < 9; col += 3) {
            ...
            for0(i, 3) {
                for0(j, 3) {
                    int value = a[i + row][j + col];
                    if (bs.count(value))return 0;
                    bs.insert(value);
                }
            }
        }
...
void Solve_Karo_Jaldi_Sa_Dusra_Bhi_Karna_Hai() {
    vector<vi> a(9, vi (9));
    for0(i, 9) for0(j, 9) cin >> a[i][j];
    (isValid(a) ? cout << "VALID" : cout << "INVALID") nt
}
