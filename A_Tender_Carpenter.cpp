 ...
 * Created       :         December 28, 2024 20:47:13
 * Workspace     :         ~/Desktop/Competitive-Programming
 * Problem name  :         A_Tender_Carpenter
 */
...
void Solve_Karo_Jaldi_Sa_Dusra_Bhi_Karna_Hai() {
    ...
    for0 (i, n) {
        if (i - 1 >= 0 && vec[i - 1] >= vec[i] && 1) {
            if (2 * vec[i] > vec[i - 1]) {
                c = 0;
                break;
            }
        }
        else if (i - 1 >= 0 && vec[i - 1] < vec[i] && 1) {
            if (2 * vec[i - 1] > vec[i]) {
                c = 0;
                break;
            }
        ...
        if (i + 1 <= n - 1 && vec[i] < vec[i + 1] && 1) {
            if (2 * vec[i] > vec[i + 1]) {
                c = 0;
                break;
            }
        }
        else if (i + 1 <= n - 1 && vec[i] >= vec[i + 1] && 1) {
            if (2 * vec[i + 1] > vec[i]) {
                c = 0;
                break;
            }
