...
ll help (ll a, ll b, ll c) {
    ll temp = b / a, ans = LLONG_MAX;
    for (int i = 1; i * i <= temp; i++) {
        if (temp % i == 0) ans = min({ans, abs(c - a * i), abs(c - a * (temp / i))});
    }
...
void Solve_Karo_Jaldi_Sa_Dusra_Bhi_Karna_Hai() {
    ...
    ll ans = 2 * d;
    if (b and a % b == 0) ans = min(ans, d + help(b, a, c));
    if (a and b % a == 0) ans = min(ans, d + help(a, b, c));
