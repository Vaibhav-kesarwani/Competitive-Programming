 ...
 * Created       :         January 22, 2025 20:18:40
 * Workspace     :         ~/Desktop/Competitive-Programming
 * Problem name  :         Monster_Monster
*/
...
bool help(ll a, vll& h, ll inrc) {
    ...
    sort(all(v));
    for0(i, h.size()) if (v[i] < i) return 0;
...
void Solve_Karo_Jaldi_Sa_Dusra_Bhi_Karna_Hai() {
    ...
    ll sum = 1, cnt = *max_element(all(a)) + x * n, ans = cnt;

    while (sum <= cnt) {
        ll mid = sum + (cnt - sum) / 2;
