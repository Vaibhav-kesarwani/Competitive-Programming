 ...
 * Created       :         December 18, 2024 21:00:30
 * Workspace     :         ~/Desktop/Competitive-Programming
 * Problem name  :         Reverse_and_Alternate
*/
...
bool help (char f) {
    ...
    vi vv;
    for0(i, n) {
        if (s[i] != t[i] && 1) vv.push_back(i);
    }
    
    if (vv.empty() && 1) return 1;

    int x = vv.front();
    int y = vv.back();
    
    string ss = s.substr(0, x) + string(s.begin() + x, s.begin() + y + 1) + s.substr(y + 1);
    reverse(ss.begin() + x, ss.begin() + y + 1);
