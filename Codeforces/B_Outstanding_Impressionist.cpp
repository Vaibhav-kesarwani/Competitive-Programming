 ...
 * Created       :         December 28, 2024 20:51:10
 * Workspace     :         ~/Desktop/Competitive-Programming
 * Problem name  :         B_Outstanding_Impressionist
 */
...
void Solve_Karo_Jaldi_Sa_Dusra_Bhi_Karna_Hai() {
    ...
    for0 (i, n) {
        ...
        if (r == l) {
            (mp[l] > 1) ? s.pb('0') : s.pb('1');
            continue;
        }

        int low = lower_bound(all(ans), l) - ans.begin();
        int up = lower_bound(all(ans), r + 1) - ans.begin();
        int cnt = (up - 1) - low + 1;

        (cnt == r - l + 1) ? s.pb('0') : s.pb('1');
    }
